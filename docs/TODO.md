...
- [x] **TODO-7** — **无依赖注入**: 已拆分 composition root 与服务注册表。`service/index.ts` 按依赖顺序构造核心服务，并通过零运行时实现依赖的 `service/registry.ts` 逐个立即发布 live binding，再构造 Provider 并按显式顺序执行 `init()`；这样后续服务的构造函数可以安全读取已发布的前置依赖。18 个服务/Provider 不再反向导入 index。当前服务服从 extension 单实例生命周期，完整 DI 容器收益不足，因此保留类型化 registry，并用 TypeScript AST 架构测试固定发布顺序、禁止实现重新导入 composition root 或让 registry 引入运行时实现依赖。
