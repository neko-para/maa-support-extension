...
- 配置 ADB/Win32/PlayCover/Gamepad/Linux 等控制器参数；Linux 控制器需要 MaaFramework ≥ 5.13.0-beta.3，当前版本过旧时在构建 runtime 阶段直接提示而非进入启动流程
- 可选在项目启动时连接已配置的控制器；控制器不可用时，可选自动发现并连接唯一匹配的 ADB、Win32 或 Gamepad 控制器。自动检测先以内存合并配置尝试连接，成功后才持久化结果；已有但不完整的 ADB 配置会被补全
- 管理任务队列（添加、删除、配置选项）
- 启动/停止任务执行
- 上传图片
- 应用预设配置
