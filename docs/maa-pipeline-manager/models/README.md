...
`LayerInfo.toggleMode()` 会从 AST 重新序列化整个任务，无法保留 JSONC 注释。该方法仅为公开 API 兼容而保留并已标记弃用，不应再用于编辑器操作。
...
- `type: 'Linux'` 的控制器会从 interface 元数据（`linux.screencap` / `linux.input`）和用户配置组装一份客户端侧 JSON（含 `pipewire_source` / `display_no` 等 client-only 标记字段），由 maa-server 连接时解析并注入 `pw_node_id` / `eis_socket_path`；`pipewire_source: 'Portal'` 尚未接入，构建时直接返回错误字符串
