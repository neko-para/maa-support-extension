...
`FsContentWatcher` 在 macOS 上默认启用 chokidar polling。chokidar 5 的 `fs.watch` 后端会为资源扫描命中的每个 Pipeline 和 PNG 创建 watcher，大型资源可能在首次 `ready` 前超过 macOS 的打开文件限制；polling 后端以较高的文件状态检查开销换取稳定的句柄数量。Linux 与 Windows 保持事件驱动默认值，调用方也可通过构造选项显式覆盖。首次扫描报错时，`FsContentWatcher` 会先关闭已经创建的 watcher，再抛出包含资源根目录且保留原始 `cause` 的错误。
