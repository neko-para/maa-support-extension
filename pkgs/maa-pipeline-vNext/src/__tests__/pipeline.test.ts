...
function fakeTaskOpts(taskName: TaskName) {
  const key = {
    type: 'string' as const,
    value: taskName,
    offset: 0,
    length: taskName.length + 2,
    children: []
  } satisfies StringNode
  return { taskName, taskKey: key }
}
...
describe('parseTaskNode — edge cases', () => {
  it('handles empty object', () => {
    ...
    const result = parseTaskNode(tree, fakeTaskOpts('Empty' as TaskName))
  ...
  it('preserves duplicate keys', () => {
    ...
    const tree = parseTreeWithoutParent(json)!
    const result = parseTaskNode(tree, fakeTaskOpts('Dup' as TaskName))
  ...
  it('recoType undefined when no recognition field', () => {
    ...
    const tree = parseTreeWithoutParent(json)!
    const result = parseTaskNode(tree, fakeTaskOpts('NoReco' as TaskName))
  ...
  it('next as single string', () => {
    ...
    const tree = parseTreeWithoutParent(json)!
    const result = parseTaskNode(tree, fakeTaskOpts('Solo' as TaskName))
  ...
  it('desc field creates doc decl', () => {
    ...
    const tree = parseTreeWithoutParent(json)!
    const result = parseTaskNode(tree, fakeTaskOpts('DocExample' as TaskName))
  ...
  it('$__mpe prefixed keys create mpe_config decl in task', () => {
    ...
    const tree = parseTreeWithoutParent(json)!
    const result = parseTaskNode(tree, fakeTaskOpts('Mpe' as TaskName))
...
describe('parseTaskNode — focus', () => {
  it('parses focus with locale ($ prefix)', () => {
    ...
    const tree = parseTreeWithoutParent(json)!
    const result = parseTaskNode(tree, fakeTaskOpts('FocusTask' as TaskName))
  ...
  it('parses focus with can_locale (non-$ string)', () => {
    ...
    const tree = parseTreeWithoutParent(json)!
    const result = parseTaskNode(tree, fakeTaskOpts('FocusTask2' as TaskName))
...
describe('parseTaskNode — freeze', () => {
  it('parses target ref inside freeze objects', () => {
    ...
    const tree = parseTreeWithoutParent(json)!
    const result = parseTaskNode(tree, fakeTaskOpts('FreezeTask' as TaskName))
